...
def get_new_dataset_information(color_image):
    ...
    json_data = data_json_tools.load_from_json(meta_plus_path)
...
def manual_data_handling():
    ...
    for i in range(masks.shape[2]):
        ...
        cv2.imshow(f"({constants.synset_names[class_ids[i]]}) - Mask {i}", visual_mask)
        cv2.imshow(f"({constants.synset_names[class_ids[i]]}) - Coords {i}", coords[:,:,i,:])
...
def nocs_util_data_handling():
    ...
    dataset = nocs_dataset.NOCSDataset(constants.synset_names, 'val', config)
    dataset.load_camera_scenes(str(camera_dataset))
    dataset.prepare(constants.class_map)
    ...
    gt_RTs, _, _, _ = nocs_utils.align(gt_class_ids, gt_mask, gt_coord, depth, constants.intrinsics,
                            constants.synset_names, None, None)
...
def test_original_data_organization():
    ...
    for name, data_handling_function in {'manual': manual_data_handling, 'nocs_util': nocs_util_data_handling}.items():
        ...
        """
        # Version 1 (drawing)
        drawn_image = tools.visualize.draw_detections(color_image, image_id, constants.intrinsics, constants.synset_names, 
        ...
        output = nocs_model.draw_detections(color_image, constants.intrinsics, constants.synset_names, bboxes, class_ids,
    ...
    data_json_tools.save_to_json(pathlib.Path.cwd() / 'first_json.json', data)
...
def test_new_data_organization():
    ...
    # Loading data
    print("Reading json data")
    json_data = data_json_tools.load_from_json(pathlib.Path.cwd() / 'first_json.json')
    instance_dict = json_data['instance_dict']
    scales = np.asarray(json_data['scales'], dtype=np.float32)
    RTs = np.asarray(json_data['RTs'], dtype=np.float32)
    ...
    output = visual_draw.draw_detections(color_image, constants.intrinsics, constants.synset_names, bboxes, class_ids,
                                         masks, coords, RTs, scores, scales, draw_coord=True, draw_tag=True, draw_RT=True)
...
def create_new_dataset(dataset_path, obj_model_dir):
    ...
    for color_image in all_color_images:
        ...
        for RT in RTs:
            ...
            perfect_projected_axes = data_manipulation.transform_3d_camera_coords_to_2d_quantized_projections(xyz_axis, RT, constants.intrinsics)
            ...
            #new_RT = data_manipulation.fix_quaternion_RT(constants.intrinsics, RT, new_RT, normalizing_factor)
            
            # Method 2 (high accuracy)
            projected_origin = perfect_projected_axes[0,:].reshape((-1, 1))
            origin_z = (np.linalg.inv(new_RT)[2, 3] * 1000)
            new_translation_vector = data_manipulation.create_translation_vector(projected_origin, origin_z, constants.intrinsics)
        ...
        output = visual_draw.draw_detections(image, constants.intrinsics, None, None, class_ids,
                                             None, None, old_RTs, None, scales, [1 for i in range(len(old_RTs))],
                                             (0,0,255), False, False, True)
        output = visual_draw.draw_detections(output, constants.intrinsics, None, None, class_ids,
